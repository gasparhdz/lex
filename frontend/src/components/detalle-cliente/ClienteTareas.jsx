...
export default function ClienteTareas({
  ...
  open = true,
  onToggle,
  // onItemClick ya no se usa para navegar; lo dejamos por compatibilidad si lo necesitás
  onItemClick,
  onEdit,
  onDetail,
  onAdd,
  ...
  const items = useMemo(() => {
    return (tareas || []).map((t) => {
      const dt = t.fechaLimite ? dayjs.utc(t.fechaLimite).local() : null;
      const vencida = !!(dt && dt.isBefore(dayjs())); // compara con “ahora”
      const fechaTxt = dt ? dt.format("DD/MM/YYYY HH:mm") : "-";
      const prioridadNombre = t?.prioridad?.nombre || "";
      const prioridadCodigo = t?.prioridad?.codigo || "";
      const ts = dt ? dt.valueOf() : Number.POSITIVE_INFINITY; // sin fecha al final
  ...
  const Content =
    visibles.length === 0 ? (
      <Alert severity="info">
        {onlyOverdue ? "No hay tareas vencidas." : "Aún no hay tareas asociadas a este cliente."}
      </Alert>
    ) : (
