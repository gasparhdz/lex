...
function computeMontoARS(c, vjGlobal) {
  ...
  const vj = Number(c.valorJusRef ?? vjGlobal ?? 0);
...
const formatMontoCuotaCell = (c, vj) => {
  const ars = Number(c.montoARS || 0);
  const jus = Number(c.jus || 0);
  if (jus > 0 && vj) {
...
export default function IngresoCuotaForm({
  ...
  valorJusNum,
  ingresoId,          // edición
  restanteARS,        // saldo global disponible (solo informativo aquí)
  value = [],
  onChange,
  ...
  const rowByHonId = useMemo(() => {
    const map = new Map();
    for (const r of value) map.set(idKey(r?.honorario?.id), r);
    return map;
  }, [value]);
    ...
    (hon, patch) => {
      ...
      if (current) {
        const next = value.map((r) =>
          idKey(r?.honorario?.id) === hid ? { ...r, ...patch } : r
        );
        onChange?.(next);
      } else {
        onChange?.([
      ...
      >
        <TableBody>
         {(honListEffective || []).map((h) => {
            ...
            const selectedCount = (rowState?.cuotasResolved || []).filter(
...
function HonorarioDetail({
  ...
  const { data: cuotas = [], isFetching } = useHonorarioCuotas(honorario.id);
  ...
  const enrichedCuotas = useMemo(() => {
    return (cuotas || [])
      ...
      .map((c) => {
        const ars = Number(c.montoARS ?? computeMontoARS(c, valorJusNum) ?? 0);
        const aplicado = Number(c.aplicadoARS ?? 0);
        const saldo = efectivoSaldoARS({ ...c, montoARS: ars, aplicadoARS: aplicado }, valorJusNum);
        return { ...c, montoARS: ars, aplicadoARS: aplicado, saldoARS: saldo };
      });
  }, [cuotas, valorJusNum]);
  ...
  // 🔰 En modo ver uso directamente las cuotas enriquecidas locales
  const allRows = viewOnly ? enrichedCuotas : (rowState?.cuotasResolved || []);
      ...
      >
              ...
              <Checkbox
                ...
                onChange={(e) => {
                  ...
                  if (!wantCheckAll) {
                    ...
                    for (const c of eligibles) delete next[idKey(c.id)];
                    setSelectedForHon({ ...next });
                    return;
                  }
                  const next = { ...selectedMap };
                  for (const c of orderByVtoNum(eligibles)) next[idKey(c.id)] = true;
                  setSelectedForHon(next);
                }}
              />
            </TableCell>
        ...
        <TableBody>
          {rowsToRender.map((c) => {
            ...
            // Deshabilito si está saldada y ESTE ingreso no aplicó nada
            const disabled = (fullyPaid && appliedHere <= 0);

            // Si está pagada por otros (disabled) => mostrar tildado gris
            const visualChecked = viewOnly ? appliedHere > 0 : (disabled ? true : !!selectedMap[cid]);
            ...
            return (
              <TableRow key={cid} hover>
                <TableCell padding="checkbox">
                  <Checkbox
                    ...
                    onChange={(e) => {
                      ...
                      const next = { ...selectedMap };
                      if (chk) next[cid] = true; else delete next[cid];
                      setSelectedForHon(next);
                    }}
                  />
                </TableCell>
                ...
                <TableCell align="right">{formatMontoCuotaCell(c, valorJusNum)}</TableCell>
                <TableCell align="right">{formatCurrency(Number(appliedHere || c.aplicadoARS || 0), "ARS")}</TableCell>
                <TableCell align="right">
